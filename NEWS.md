...
- Performance: The unified diff shown by `--list --diff` and stored in
  `mutants.out/mutants.json` for every mutant is now generated by diffing only
  the lines around the mutation, rather than the whole file against a whole
  mutated copy of it. Because a diff is generated for every mutant before any
  of them is tested, that cost used to grow with the product of the number of
  mutants in a file and the file's size: on a 300 KB file with 7,600 mutants it
