...
- New: `--detect-equivalent-mutants` finds mutants that provably cannot be caught by any test, by comparing the compiled test binary for each mutant, byte for byte, against the baseline and against earlier mutants (Trivial Compiler Equivalence). Matching mutants are reported as `equivalent` and skip the test phase, instead of being (mis)reported as `caught` or `missed`. This forces `-Cdebuginfo=0` for all builds, and typically finds only a few percent of mutants at the default (unoptimized) profile; the yield is larger with an optimized `--profile`. When the test tool is `cargo` (the default), this also runs `cargo test` with `--lib --bins --tests` instead of testing doc tests too: doc tests never contribute to the compiled-binary fingerprint this feature compares, so leaving them in the run would risk skipping a mutant that only a doc test would have caught; a mutant caught only by a doc test is now reported `missed`, not `equivalent`. See [equivalent mutants](https://mutants.rs/equivalent-mutants.html).

- New: `--skip-uncovered` prefilters mutants using line coverage measured during the baseline test run (via `-Cinstrument-coverage`, which needs the `llvm-tools` rustup component): a mutant on a line no test executes can't be caught by any test, so it's known before spending a build and a test run on it. Such mutants are reported `uncovered`, listed in `mutants.out/uncovered.txt`, and still count as a found problem, exactly like `missed`. Like `--detect-equivalent-mutants`, this also runs `cargo test` with `--lib --bins --tests` instead of testing doc tests, because `-Cinstrument-coverage` doesn't reach rustdoc: a function exercised only by a doc test is reported `uncovered`. Incompatible with `--baseline=skip` and `--check`, which don't run a baseline test to measure coverage from.

- New: Six new mutation genres: the condition of an `if` and of a `while` are replaced with fixed values; `true` and `false` literals are replaced with each other; statements whose value is discarded (calls, method calls, plain assignments) are deleted; the value of an explicit `return` is replaced; and a function returning an enum declared in the same file is replaced with each of that enum's unit variants. See [mutation patterns](https://mutants.rs/mutants.html).

- New: More return types are mutated: `char`, `PathBuf`, `Utf8PathBuf`, `OsString`, `Duration`, `cmp::Ordering`, and `Range<T>`; and `Result<T, E>` now also generates `Err` values when `E` is a type whose values can be constructed directly, such as `String`.

- New: `<=` is also mutated to `<`, and `>=` to `>`, closing the boundary pair for those operators.

- Fixed: No mutants are generated for return types that have no value that can be constructed here (`dyn Trait`, `Pin`, `NonNull`, `Instant`, `SystemTime`), instead of unviable `Default::default()` mutants.
