...
#[test]
fn genres_tree_is_well_tested() {
    let tmp_src_dir = copy_of_testdata("genres");
    run()
        .arg("mutants")
        .args(["--no-times", "--no-shuffle", "-v", "-V"])
        .current_dir(tmp_src_dir.path())
...
/// debug log looks like it's using multiple threads. A `--check` run is
/// enough to exercise the per-job worker threads, since each thread builds
/// its own copy of the tree regardless of whether tests are actually run.
#[test]
fn jobs_option_accepted_and_causes_multiple_threads_during_check() {
...
fn warn_about_too_many_jobs() {
    // The "too many jobs" warning is emitted while validating options, before
    // any mutants are built or tested, so `--list` is enough to trigger it.
    let testdata = copy_of_testdata("small_well_tested");
    run()
        .arg("mutants")
        .arg("--list")
...
fn no_config_option_disables_config_file_so_error_value_is_not_generated() {
    // In this case, the config file is not loaded, so no mutant returns the
    // error value it configures. The tree's deliberately weak test does leave
    // other mutants alive, so problems are found and reported.
    ...
    run()
        ...
        .code(2)
        .stderr("")
        .stdout(predicate::str::contains("eyre").not())
        .stdout(predicate::function(|stdout: &str| {
            insta::assert_snapshot!(stdout);
            true
        }));
...
fn check_tree_with_mutants_skip() {
    ...
    run()
        ...
        .stdout(indoc! { r"
            ...
            ok       src/lib.rs:18:12: replace should_stop() with true in controlled_loop
            ok       src/lib.rs:21:12: replace start.elapsed() > Duration::from_secs(60 * 5) with true in controlled_loop
            ok       src/lib.rs:21:12: replace start.elapsed() > Duration::from_secs(60 * 5) with false in controlled_loop
            ok       src/lib.rs:21:28: replace > with == in controlled_loop
            ok       src/lib.rs:21:28: replace > with < in controlled_loop
            ok       src/lib.rs:21:28: replace > with >= in controlled_loop
            ok       src/lib.rs:21:53: replace * with + in controlled_loop
            ok       src/lib.rs:21:53: replace * with / in controlled_loop
            9 mutants tested: 9 succeeded
            "})
        .stderr("");
    assert_eq!(
        outcome_json_counts(&tmp_src_dir),
...
fn cross_package_tests() {
    ...
    // Using the wrong package name gives a warning. The warning is emitted
    // while resolving which package to test, before any mutants are built,
    // so `--check` is enough to trigger it without testing mutants.
    run()
        .args(["mutants", "--test-package=tests", "--check"])
        .arg("-d")
        .arg(path.join("lib"))
        .env_remove("RUST_BACKTRACE")
        .assert()
